        ...
        try:
            if self.id == '0000000000' or self.id == '0000000':
                if self.number == '-':
                    ...
                    res = json.loads(requests.get(esi_url,verify='./aws.cloud.oncor.crt').text)
            ...
            res = json.loads(requests.get(validate_url,verify='./aws.cloud.oncor.crt').text)
            if res.get('Error',False):
                return 'Bad_ID'
            if res.get('validateAccountResponse',False):
                if res['validateAccountResponse']['account']['status'] =='ACTIVE':
                    
                    res = json.loads(requests.get(url,verify='./aws.cloud.oncor.crt').text)
